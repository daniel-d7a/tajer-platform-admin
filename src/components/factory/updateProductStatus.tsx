...
export default function UpdateProductStatus({ onClose, factoryBatchId, product }: UpdateProductStatusProps) {
  ...
  const handleUpdateProductStatus = async () => {
    try {
      setLoading(true);
      await fetch(`https://tajer-backend.tajerplatform.workers.dev/api/admin/factory-batches/${factoryBatchId}/items`, {
        method: 'PATCH',
        credentials: 'include',
        headers: {
