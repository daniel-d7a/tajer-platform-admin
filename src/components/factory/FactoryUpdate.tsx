  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (factory && factory.id) {
        url = `https://tajer-platform-api.eyadabdou862.workers.dev/api/admin/factories/${factory.id}`;
        method = "PUT";
      } else {
        url =
      ...
      if (response.ok) {
        toast.success(
          factory ? "تم تحديث بيانات المصنع بنجاح" : "تمت إضافة مصنع جديد بنجاح"
        );
        onClose();
      } else {
        const errorData = await response.json();
        toast.error(
          errorData.message || "فشل الحفظ، يرجى المحاولة في وقت لاحق"
        );
      }
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto z-50">
      <div className="relative mx-auto p-5 border w-11/12 max-w-4xl shadow-lg rounded-md bg-white my-8">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-2">
            <label htmlFor="name_ar" className="font-medium">
              اسم المصنع بالعربية
            </label>
            <input
              value={updatedData.name_ar}
              onChange={(e) =>
                setUpdatedData({ ...updatedData, name_ar: e.target.value })
              }
              id="name_ar"
              className="border-2 border-gray-300 rounded-md p-2 w-full"
          ...
          <div className="flex flex-col gap-2">
            <label htmlFor="discountAmount" className="font-medium">
              مقدار الخصم علي منتجات المصنع د.أ
            </label>
            <input
          ...
          <div className="flex flex-col gap-2">
            <label htmlFor="discountType" className="font-medium">
              نوع الخصم علي المصنع
            </label>
            <select
              value={updatedData.discountType}
              onChange={(e) =>
                setUpdatedData({ ...updatedData, discountType: e.target.value })
              }
              id="discountType"
              className="border-2 border-gray-300 rounded-md p-2 w-full"
              disabled={loading}
