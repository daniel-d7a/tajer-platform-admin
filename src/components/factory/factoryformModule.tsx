    ...
    const fetchData = async () => {
      try {
        const data = await fetch(
          `https://tajer-backend.tajerplatform.workers.dev/api/admin/factory-batches/${factory.id}`,
          { credentials: "include" }
        );
        ...
        <div className="relative mx-auto p-5 border w-11/12 max-w-3xl shadow-lg overflow-x-scroll rounded-md bg-white">
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            ...
            <div>
              ...
              <p className="text-gray-600">{dataFactory?.status=== 'ORDERED_FROM_FACTORY'
                ? 'مطلوب من المصنع'
                            ...
                            <tbody>
                              {factoryItem.items.map((item) => (
                                <tr key={item.id} className="bg-white hover:bg-gray-300">
                                  ...
                                  <td className="px-2 py-2 text-right font-semibold">
                                    {item.product.unitType === 'piece_only' ? 'قطعه فقط' :  item.product.unitType === 'pack_only' ? "حزمه فقط" : "قطعه او حزمه"}
                                  </td>
