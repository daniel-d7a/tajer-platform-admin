...
export default function Banners() {
  ...
  const fetchBanners = async () => {
    try {
      const response = await fetch('https://tajer-backend.tajerplatform.workers.dev/api/admin/banners', {
        credentials: 'include'
      });
      if (response.ok) {
  ...
  const handleDeleteBanner = async (id: number) => {
    ...
    try {
      const response = await fetch(`https://tajer-backend.tajerplatform.workers.dev/api/admin/banners/${id}`, {
        method: 'DELETE',
        credentials: 'include'
      });
      if (response.ok) {
  ...
  const handleSaveBanner = async (formData: FormData) => {
    try {
      ...
      const url = selectedBanner?.id 
        ? `https://tajer-backend.tajerplatform.workers.dev/api/admin/banners/${selectedBanner.id}`
      ...
      if (response.ok) {
        toast.success(selectedBanner?.id ? 'تم تحديث البانر بنجاح' : 'تم إضافة البانر بنجاح');
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {bannersData.map((banner) => (
            <div key={banner.id} className="relative group bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition-all duration-300">
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4 text-white">
                  ...
                  {banner.link && (
                    <div className="text-xs opacity-90 truncate mt-1">
                      {banner.link.replace('https://tajer-jo.com/en/products/', 'منتج: ')}
                    </div>
                  )}
                </div>
        ...
        <div className="flex flex-col items-center justify-center h-64 bg-gray-50 rounded-xl border-2 border-dashed border-gray-300">
          <div className="text-center">
            ...
            <p className="text-gray-500 text-lg mb-2">لا يوجد أي بانرات حتى الآن</p>
