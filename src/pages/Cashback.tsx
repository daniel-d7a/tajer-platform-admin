...
import { Edit, Eye, Trash2,PlusCircle, Calendar} from "lucide-react";
...
export default function CashBack() {
  ...
  const [selectedCashBack, setSelectedCashBack] = useState<CashbackCampaign | null>(null);
  ...
  const handleDelete =  async (item :CashbackCampaign)=> {
    if(window.confirm('هل ترغب حقا في حذف ' + item.name)){
       try{
        const data = await fetch(`https://tajer-backend.tajerplatform.workers.dev/api/cashback/cashback/campaigns/${item.id}`, {
        credentials:'include',
      method: 'DELETE'
})
      if(!data.ok){
...
const columns = [
    ...
    },
    {
      ...
      <div className="flex space-x-2">
        <button   
          onClick={() => {setOpenCashBackDetails(true) ; setSelectedCashBack(item)}}
        ...
        <button   
          onClick={() => {setOpenCashBackEdite(true) ; setSelectedCashBack(item)}}
  ...
  return (
   ...
   <div className='w-full flex justify-end gap-2 mb-4'>
        <button className='bg-[hsl(var(--primary))] text-white px-4 py-2 rounded-md cursor-pointer flex items-center' 
              onClick={() => {
      ...
      {OpencashBackDetails && selectedCashBack && (
        <CashBackDetails items={selectedCashBack} onClose={() => setOpenCashBackDetails(false)} />
      )}
        {OpencashBackEdite  && (
        <CashBackEdit onSuccess={() => {
          toast.success('تم تحديث العرض بنجاح !')
          cashBack();
        }} items={selectedCashBack} onClose={() => setOpenCashBackEdite(false)} />
      )}
      <DataTable  
        data={data}
        columns={columns}
